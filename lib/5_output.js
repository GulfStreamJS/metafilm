...
module.exports = params => {
    ...
    return new Promise(resolve => {
                                    ...
                                    if (output[key]) {
                                        processed[key].forEach(str => {
                                            ...
                                            output[key].forEach((out, i) => {
                                                ...
                                                if (
                                                    (
                                                        str.poster && str.poster.md.indexOf('tvmaze') + 1 ||
        ...
        if (params.debug) {
            require('fs').writeFileSync(require('path').join(__dirname, '../test/' + (typeof params.debug === 'string' ? 'params_' + params.debug : 'params') + '.json'), JSON.stringify(params, null, 2));
            require('fs').writeFileSync(require('path').join(__dirname, '../test/' + (typeof params.debug === 'string' ? 'output_' + params.debug : 'output') + '.json'), JSON.stringify(output, null, 2));
        }
