...
module.exports = params => {
    ...
    return Promise.resolve(params)
        .then(params => {
            if (params.progress) bar.tick(1, {title: 'DOUBAN SEASON'});
            return params;
        })
        .then(douban_season)
        .then(params => {
            if (params.progress) bar.tick(1, {title: 'DOUBAN ID'});
            return params;
        })
        .then(douban_id)
        .then(params => {
            if (params.progress) bar.tick(1, {title: 'TMDB ID'});
            return params;
        })
        .then(tmdb_id)
        .then(params => {
            if (params.progress) bar.tick(1, {title: 'IMDB ID'});
            return params;
        })
        .then(imdb_id)
        .then(params => {
