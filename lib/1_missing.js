...
module.exports = params => {
    ...
    const tvmaze_id = params => {
        ...
        let url = 'https://api.tvmaze.com/lookup/shows?imdb=tt' + params.imdb_id;
        let headers = {'User-Agent': useragent(), 'maxRedirects': 0, 'validateStatus': null};
        let proxy = params.proxy || null;
        ...
        return new Promise(resolve => {
            return axios({url, headers, proxy})
                .then(res => {
                    if (
                        res.status &&
                        (res.status === 301 || res.status === 302 || res.status === 200)
                    ) {
                        if (res.headers && res.headers.location && /\/shows\/[0-9]{1,11}/i.test(res.headers.location)) {
                            params.tvmaze_id = res.headers.location
                                .toString()
                                .replace(/[^0-9]/ig, '');
                        } else if (res.request && res.request.res && res.request.res.responseUrl && /\/shows\/[0-9]{1,11}/i.test(res.request.res.responseUrl)) {
                            params.tvmaze_id = res.request.res.responseUrl
                                .toString()
                                .replace(/[^0-9]/ig, '');
                        } else if (res.request && res.request.path && /\/shows\/[0-9]{1,11}/i.test(res.request.path)) {
                            params.tvmaze_id = res.request.path
                                .toString()
                                .replace(/[^0-9]/ig, '');
                        }
                    }
    ...
    return Promise.resolve(params)
        ...
        .then(imdb_id)
        .then(params => {
            if (params.gs) bar.tick(1, {title: 'TVMAZE ID'});
            return params;
        })
        .then(tvmaze_id)
        .then(params => {
